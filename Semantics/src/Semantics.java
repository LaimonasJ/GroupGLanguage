...
public class Semantics
{
      ...
      while (this.parserTree.indexOf("<struct-or-func-decl>") > 0)
      {
         ...
         switch (this.parserTree.get(0))
         {
            ...
            case "<struct-declaration>":
               ...
               Type newType = new Type(this.parserTree.get(2));
               System.out.println("struct " + newType.name);
               boolean error = false;
               if (lookupType(newType.name) != null)
               {
                  System.out.println("ERROR: type " + newType.name + " is already defined!");
                  error = true;
               }
               //System.out.println(this.parserTree.indexOf("<declaration>") + " " + this.parserTree.indexOf("</struct-declaration>"));
               while (this.parserTree.indexOf("<declaration>") > 0 && (this.parserTree.indexOf("<declaration>") < this.parserTree.indexOf("</struct-declaration>")))
               {
                  int index = this.parserTree.indexOf("<declaration>");
                  String typeName = getTypeName(index + 1);
                  if (lookupType(typeName) == null)
                  {
                     System.out.println("ERROR: type " + typeName + " is not defined!");
                     error = true;
                  } else
                  {
                     Type type = new Type(typeName);
                     for (int i = this.parserTree.indexOf("<identifier-list>"); i < this.parserTree.indexOf("</identifier-list>"); i++)
                     {
                        if (this.parserTree.get(i).equals("<identifier-list>"))
                        {
                           String name = this.parserTree.get(i + 1);
                           if (newType.variableList.contains(new Variable(name, type)))
                           {
                              System.out.println("ERROR: identifier " + name + " is already defined in struct " + newType.name + "!");
                              error = true;
                           } else
                           {
                              newType.variableList.add(new Variable(name, type));
                           }
                        }
                     }
                  }
                  this.parserTree.subList(this.parserTree.indexOf("<declaration>"), this.parserTree.indexOf("</declaration>") + 1).clear();
               }
               if (error == false)
               {
                  typeList.add(newType);
               }
               this.parserTree.subList(this.parserTree.indexOf("<struct-declaration>"), this.parserTree.indexOf("</struct-declaration>") + 1).clear();
               break;
            default:
   ...
   public static Variable findPrimary(int index)
   {
      ...
      switch(primaryValue)
      {
         case "<function-call>":
            ...
            break;
         case "(":
            rez = findExpression(index + 2);
            System.out.println(rez.type);
            break;
         default:
   ...
   public String getTypeName(int index)  //gets type name from parserTree. index = index of "declarator"
   {
      String typeName;
      if (parserTree.get(index + 1).equals("<type>"))
      {
         typeName = parserTree.get(index + 2);
      } else
      {
         typeName = parserTree.get(index + 1);
      }
      parserTree.subList(index, parserTree.indexOf("</declarator>")).clear();
      return typeName;
   }
}
